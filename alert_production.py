...
#Connect to influxdb
print('***********************Chuyển dữ liệu ALERT PRODUCTION từ influxdb sang SQL Server****************************')
print("Loading...")
...
#------------------PHẦN ALERT------------------------
results = client.query('SELECT  "_abbr_0", "_prior", "_text_0", "_ack_time", "_ack_user", "_ackable", "_direction", "alarmvalue_bool", "alarmvalue_float", "alarmvalue_int", "_comment", "_class", "_panel", "name" FROM "ALERT"."ALERT" WHERE ("name" =~ /System1:PRO_*/ AND "time" >= \''+ start_datetime +'\' AND "time" <= \'' + end_datetime + '\')')
...
df.to_sql('ALERT_PRODUCTION', con = engine, if_exists = 'append', chunksize=1000, index=False)
print("Hoàn tất")
